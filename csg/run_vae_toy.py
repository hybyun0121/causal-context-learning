...
# Synthetic Data
from utils_data import HighDimSCMSyntheticDataset
...
def get_frame(discr, gen, dc_vars, device = None, discr_src = None):
    ...
    if dc_vars['cond_prior']:
        prior_std = mlp.create_prior_from_json("MLPs1t", discr, actv=dc_vars['actv_prior'],jsonfile=dc_vars['mlpstrufile']).to(device)
        std_s = prior_std.std_s1t
    else:
        std_s = dc_vars['sig_s']

    frame = SupVAE(shape_s=shape_s, shape_x=shape_x, dim_y=dc_vars['dim_y'],
                   mean_x1s=gen.x1s, std_x1s=dc_vars['pstd_x'], logit_y1s=discr.y1s,
                   std_y1s=discr.std_y1s,
                   tmean_s1x=None, tstd_s1x=None,
                   mean_s1x=mean_s, std_s1x=std_s1x,
                   mean_s=dc_vars['mu_s'], std_s=std_s,
                   learn_tprior=mode == 'da',
                   src_mvn_prior=dc_vars['src_mvn_prior'],
                   tgt_mvn_prior=dc_vars['tgt_mvn_prior'],
                   device=device)
...
def save_ckpt(ag, res, ckpt, dc_vars, discr, gen, frame, opt, IS_OOD=True, filename=None):
    ...
    if filename is None:
        filename = unique_filename(
                dirname + (f"vae_ood_toy" if IS_OOD else f"da_toy"), ".pt", n_digits = 3
            ) if ckpt is None else ckpt['filename']

    dc_vars = edic(locals()).sub([
            'dirname', 'filename', 'testdoms',
            'shape_x', 'shape_s', 'dim_x', 'dim_y']) | ( edic(vars(ag)) - {'testdoms'}
...
def add_ce_loss(lossobj, celossfn, ag):
    ...
    def lossfn(*x_y_maybext_niter):
        ...
        if ag.wsup:
            loss += ag.wsup * shrink_sup(x_y_maybext_niter[-1]) * celossfn(x_y_maybext_niter[:-1])
        return loss
    return lossfn
...
def ood_methods(discr, frame, ag, dim_y):
    ...
    lossobj = frame.get_lossfn(ag.n_mc_q, ag.reduction, wlogpi=ag.wlogpi)
...
def process_continue_run(ag):
    ...
    if ag.init_model not in {"rand", "fix"}: # continue running
        ...
        for k in vars(ag):
            if k not in {"testdoms", "n_epk", "gpu", "deploy"}: # use the new final number of epochs
                setattr(ag, k, ckpt[k])
...
class InferenceCollector:
    ...
    def collect_batch(self, data_bat, s_hat, x_hat, y_hat):
    ...
    def to_dataframe(self):
        ...
        cs = np.concatenate(self.cs, axis=0)
        ss = np.concatenate(self.ss, axis=0)
...
def inference_variables(lossfn_eval, frame, discr, gen, data_loader, device, phase='val', ta_data_loader=None, IS_OOD=True, n_mc=0,
    ...
    for i_bat, data_bat in enumerate(data_loader if IS_OOD else zip_longer(data_loader, ta_data_loader), start=1):
        ...
        if phase == 'test':
            if n_mc == 0:
                with tc.no_grad():
                    if sample_cs_draw:
                        s_samples = frame.q_s1x.draw((1,), {'x':xs})
                        s_hat = s_samples['s'].squeeze(0)
                    else:
                        s_hat = frame.q_s1x.mean({'x':xs})['s']

                    if gen_probs:
                        x_hat = frame.p_x1s.mean({'s':s_hat})['x']
                        y_hat = frame.p_y1s.mean({'s':s_hat})['y']
            ...
            collector.collect_batch(data_bat, s_hat, x_hat, y_hat)
        else:
            if n_mc == 0:
                with tc.no_grad():
                    if sample_cs_draw:
                        s_samples = frame.q_s1x.draw((1,),{'x':xs})['s']
                        s_hat = s_samples['s'].squeeze(0)
                    else:
                        s_hat = frame.q_s1x.mean({'x':xs})['s']
        ...
        total_c_recon_loss += F.mse_loss(s_hat, cs, reduction='none').mean(dim=1).sum()
        total_s_recon_loss += F.mse_loss(s_hat, ss, reduction='none').mean(dim=1).sum()
    ...
    avg_c_recon_loss = total_c_recon_loss / len(data_loader.dataset)
    avg_s_recon_loss = total_s_recon_loss / len(data_loader.dataset)
    ...
    if phase == 'test':
        ...
        return avg_loss, avg_x_recon_loss, avg_y_recon_loss, avg_e_recon_loss, avg_c_recon_loss, avg_s_recon_loss, avg_cossim_x, avg_cossim_y, avg_cossim_e, results_df
    return avg_loss, avg_x_recon_loss, avg_y_recon_loss, avg_e_recon_loss, avg_c_recon_loss, avg_s_recon_loss, avg_cossim_x, avg_cossim_y, avg_cossim_e
    ...
    for epk in range(epk0, ag.n_epk+1):
        ...
        if epk % ag.eval_interval == 0:
            ...
            with tc.no_grad():
                ...
                if IS_OOD:
                    loss_eval, avg_x_recon_loss, avg_y_recon_loss, avg_e_recon_loss, avg_c_recon_loss, avg_s_recon_loss, avg_cossim_x, avg_cossim_y, avg_cossim_e = inference_variables(lossfn_eval, frame, discr, gen, val_src_loader, device, gen_probs=ag.gen_probs)
                # else:
                ...
                wandb.log({
                    ...
                    'C_Recon_Loss': avg_c_recon_loss.item(),
                    'S_Recon_Loss': avg_s_recon_loss.item(),
                    'Val_Cossim_X': avg_cossim_x.item(),
                    'Val_Cossim_Y': avg_cossim_y.item(),
                    # 'Val_Cossim_E': avg_cossim_e.item()
                })
...
def main_depoly(ag, ckpt, archtype, shape_x, dim_y,
    ...
    with tc.no_grad():
        ...
        loss_eval_ID, avg_x_recon_loss_id, avg_y_recon_loss_id, avg_e_recon_loss_id, avg_c_recon_loss_id, avg_s_recon_loss_id, avg_cossim_x, avg_cossim_y, avg_cossim_e, results_ID = inference_variables(lossfn_eval, frame, discr, gen, tr_src_loader, device, phase='test', gen_probs=ag.gen_probs)
        loss_eval_OOD, avg_x_recon_loss_ood, avg_y_recon_loss_ood, avg_e_recon_loss_ood, avg_c_recon_loss_ood, avg_s_recon_loss_ood, avg_cossim_x, avg_cossim_y, avg_cossim_e, results_OOD = inference_variables(lossfn_eval, frame, discr, gen, ls_ts_tgt_loader, device, phase='test', gen_probs=ag.gen_probs)
        ...
        print(f"Mode '{ag.mode}': ID C Recon Loss = {avg_c_recon_loss_id:.3f}")
        print(f"Mode '{ag.mode}': ID S Recon Loss = {avg_s_recon_loss_id:.3f}")
        ...
        print(f"Mode '{ag.mode}': OOD C Recon Loss = {avg_c_recon_loss_ood:.3f}")
        print(f"Mode '{ag.mode}': OOD S Recon Loss = {avg_s_recon_loss_ood:.3f}")
        ...
        wandb.log({
            ...
            'ID_C_Recon_Loss': avg_c_recon_loss_id.item(),
            'OOD_C_Recon_Loss': avg_c_recon_loss_ood.item(),
            'ID_S_Recon_Loss': avg_s_recon_loss_id.item(),
            'OOD_S_Recon_Loss': avg_s_recon_loss_ood.item(),
            # 'ID_E_Recon_Loss': avg_e_recon_loss_id.item(),
            # 'OOD_E_Recon_Loss': avg_e_recon_loss_ood.item()
        })

    results_ID.to_pickle(f"./results/toy_exp/results_{ckpt['filename'].split('/')[0]}_{ckpt['filename'].split('/')[1]}_{ckpt['filename'].split('/')[-1].split(".")[0]}_ID.pkl")
    results_OOD.to_pickle(f"./results/toy_exp/results_{ckpt['filename'].split('/')[0]}_{ckpt['filename'].split('/')[1]}_{ckpt['filename'].split('/')[-1].split(".")[0]}_OOD.pkl")
...
def get_parser():
    ...
    parser.add_argument("--sub_task", action = "store_true")
...
if __name__ == "__main__":
    ...
    # Dataset
    df_ID = pd.read_pickle(f"../data/synthetic_dataset/{ag.traindom}_ID.pickle")
    df_OOD = pd.read_pickle(f"../data/synthetic_dataset/{testdoms}_OOD.pickle")
